...
constrained both by small, fragmented samples and by a feature-to-sample ratio that makes locus
selection challenging. We addressed geographic assignment from SNP data through cross-species
transfer learning. We first finetunned DNABERT-2 \citep{zhou2024dnabert} on six felid reference
assemblies, and then used the resulting foundation model to compute a label-free Variant Effect
Score (VES) for each SNP, a measure of how surprising the alternate allele is in the felid genomic
context. These scores initialized per-locus learnable gates in a genotype-matrix multilayer
...
successive gains in median error, from $199$ to $174$\,km and from $174$ to $149$\,km. Taken
together, these results show that a foundation model pre-trained across the Felidae can supply extra
signal during feature selection that a small, fragmented jaguar sample cannot provide on its own.
\end{abstract}

% Introduction
\section{Introduction}

\TODO{Intro condensation: I copyied most of this from the proposal, looks too long.}
...
\citep{battey2020predicting} and GeoGenIE \TODO{need the GeoGenIE citation, Martin et al. 2025}. The
network took as input a per-individual vector of allele counts, one entry per retained
...
\TODO{need Optuna citation}, treating the
...
defaults with the same Optuna hyperparameter search we used for those experiments \TODO{same Optuna
citation as above}, again with \texttt{ves\_mode}${}=$\texttt{none}. This isolates the
...
coordinates via spatial polygon lookup, but \TODO{couldn't find the polygon-lookup code anywhere in
the repo, so not sure this part actually ran.}
...
\texttt{accelerate} handling distributed training and mixed precision. \TODO{still need citations
for the main libraries (torch, transformers, etc)}
...
scikit-learn
  & \(\geq\)1.8.0
  & Cross-validation \TD{not sure sklearn is really used here, README says so but I can't find it
  imported anywhere} \\
...
einops & \(\geq\)0.8.2 & \TD{not sure what einops is used for} \\
\bottomrule
\end{tabular}
\end{table*}
...
\TODO{need to add the actual GPU/hardware used and how long the runs took (foundation pre-training,
VES scoring, the two Optuna searches) -- don't have these numbers written down anywhere, have to dig
through old logs.}
...
confidence intervals to any figure. \TODO{should add a bootstrap CI and a Wilcoxon test on the VES
vs no-VES errors, can probably compute both from \texttt{loocv\_predictions.json}.}
...
species \TODO{need the exact within-500km number from \citet{lazzari2025development}, only have it
from the README not the actual output files}.
...
\TODO{same Optuna citation as above, Akiba et al. 2019}, holding the loss, the LOOCV protocol,
...
within 500\,km from 67.3\% to 89.1\%. \TODO{still need to make the error-distribution map}

% Discussion
\section{Discussion}

\TODO{write the discussion -- bayesopt unlocking locator on small samples, the felid foundation
model as a reusable asset, VES as a label-free alternative to F\_ST, transfer learning helping most
where data is scarcest. also limitations and future work.}

% back matter
\section*{Author Contributions}
\TODO{add author contributions (CRediT roles)}

\section*{Acknowledgements}
\TODO{acknowledgements -- funders, LBGM-PUCRS, field collaborators}

\section*{Ethics Statement}
\TODO{need permit numbers and ethics approval info for the jaguar samples, don't have these on hand}
...
evaluation pipeline --- is available in the accompanying repository \TODO{add the repo link once
it's public}.

\TODO{some spots name the author inline (e.g. "Battey and colleagues (2020)") on top of the citep,
so it double prints. go through and fix these to citet where that happens.}

\bibliography{bibliography}

\section*{Supporting Information}
\TODO{only need this section if there end up being supplementary tables/figures}
